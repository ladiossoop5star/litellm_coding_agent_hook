...
TOOL_RESULT_IMAGE_HISTORY_PLACEHOLDER = "[Historical tool image omitted to preserve context]"
...
def _split_images_from_tool_message(
    ...
    if not isinstance(content, list):
        return message, ()
    image_parts = tuple(
        cast(ChatCompletionImageObject, part)
        for part in content
    ...
    return cast(AllMessageValues, rewritten), image_parts
...
def _strip_images_from_historical_tool_message(
    message: AllMessageValues,
) -> AllMessageValues:
    rewritten, image_parts = _split_images_from_tool_message(message)
    if not image_parts:
        return rewritten

    content = rewritten.get("content")
    if content == TOOL_RESULT_IMAGE_PLACEHOLDER:
        new_content: Any = TOOL_RESULT_IMAGE_HISTORY_PLACEHOLDER
    elif isinstance(content, list):
        history_part = ChatCompletionTextObject(
            type="text", text=TOOL_RESULT_IMAGE_HISTORY_PLACEHOLDER
        )
        new_content = [*content, history_part]
    else:
        new_content = TOOL_RESULT_IMAGE_HISTORY_PLACEHOLDER

    return cast(AllMessageValues, {**rewritten, "content": new_content})


def _strip_images_from_historical_tool_message_run(
    run: Iterable[AllMessageValues],
) -> List[AllMessageValues]:
    return [_strip_images_from_historical_tool_message(message) for message in run]


def hoist_images_from_tool_messages(
    messages: List[AllMessageValues],
) -> List[AllMessageValues]:
    """Expose only the newest tool-image run and compact older image results.

    Claude clients replay every historical tool result on each request. Hoisting
    every image therefore makes request bytes and vision tokens grow without
    bound. The newest consecutive tool-message run remains visible to the model;
    images from older runs are replaced by a deterministic text placeholder.
    """
    if not any(_tool_message_carries_image(message) for message in messages):
        return messages

    grouped_runs = [
        (is_tool_run, tuple(run))
        for is_tool_run, run in groupby(
            messages, key=lambda message: message.get("role") == "tool"
        )
    ]
    newest_image_run_index = max(
        index
        for index, (is_tool_run, run) in enumerate(grouped_runs)
        if is_tool_run and any(_tool_message_carries_image(message) for message in run)
    )

    return [
        rewritten_message
        for index, (is_tool_run, run) in enumerate(grouped_runs)
        for rewritten_message in (
            _hoist_images_in_tool_message_run(run)
            if index == newest_image_run_index
            else _strip_images_from_historical_tool_message_run(run)
            if is_tool_run
            else run
        )
    ]
